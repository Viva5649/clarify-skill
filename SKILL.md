---
name: clarify-thought
description: 用于把模糊想法拆成可执行指令，或把说不清楚的纠结与决策澄清出来。当用户说「帮我理清一下」「我想做个事但说不清楚」「帮我澄清」「我有点纠结」或表达类似的模糊需求、内心困扰、冲动决策时，也应主动触发。
---

# /clarify-thought — 命题分解与决策澄清系统
...
- `/clarify-thought` — 自动判断进入哪个模式
- `/clarify-thought prompt` 或 「帮我理清一下」「我想做个事但说不清楚」 → 强制进入 Prompt 模式
- `/clarify-thought decision` 或 「帮我澄清」「我有点纠结」 → 强制进入 Decision 模式

**模式判断规则**：
- 用户描述的是一个**想让 AI 做的事** → Prompt 模式
