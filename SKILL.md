---
name: clarify
description: 用于把模糊想法拆成可执行指令，或把说不清楚的纠结与决策澄清出来。用户明确输入 `/clarify`、`/clarify prompt`、`/clarify decision` 时必须使用；当用户说「帮我理清一下」「我想做个事但说不清楚」「帮我澄清」「我有点纠结」或表达类似的模糊需求、内心困扰、冲动决策时，也应主动触发。
---
